...
CREATE TABLE `alarm_item` (
	...
	`opc_item_id_gone` INT(10) UNSIGNED NOT NULL COMMENT 'I am the OPC address that the GONE statement will be written to. Write addresses are unique.',
	...
	`opc_item_id_came` INT(10) UNSIGNED NOT NULL COMMENT 'I am the OPC address the CAME date and time will be written to. Write addresses are unique.',
	PRIMARY KEY (`alarm_item_id`) USING BTREE,
	UNIQUE INDEX `alarm_item_to_opc_item_gone_unique` (`opc_item_id_gone`) USING BTREE,
	...
	UNIQUE INDEX `alarm_item_to_opc_item_came_unique` (`opc_item_id_came`) USING BTREE,
	UNIQUE INDEX `alarm_group_id_alarm_name_unique` (`alarm_group_id`, `alarm_item_name`) USING BTREE,
	INDEX `alarm_item_to_opc_item_read` (`opc_item_id_read`) USING BTREE,
	INDEX `alarm_item_to_opc_item_alarm` (`opc_item_id_alarm`) USING BTREE,
	INDEX `alarm_item_to_opc_item_came` (`opc_item_id_came`) USING BTREE,
	INDEX `alarm_item_to_opc_item_gone` (`opc_item_id_gone`) USING BTREE,
	INDEX `alarm_item_to_opc_item_ack` (`opc_item_id_ack`) USING BTREE,
	INDEX `alarm_group_id` (`alarm_group_id`) USING BTREE,
	CONSTRAINT `alarm_item_to_alarm_group` FOREIGN KEY (`alarm_group_id`) REFERENCES `nox`.`alarm_group` (`alarm_group_id`) ON UPDATE NO ACTION ON DELETE NO ACTION,
	CONSTRAINT `alarm_item_to_opc_item_ack` FOREIGN KEY (`opc_item_id_ack`) REFERENCES `nox`.`opc_item` (`opc_item_id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `alarm_item_to_opc_item_alarm` FOREIGN KEY (`opc_item_id_alarm`) REFERENCES `nox`.`opc_item` (`opc_item_id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `alarm_item_to_opc_item_came` FOREIGN KEY (`opc_item_id_came`) REFERENCES `nox`.`opc_item` (`opc_item_id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `alarm_item_to_opc_item_gone` FOREIGN KEY (`opc_item_id_gone`) REFERENCES `nox`.`opc_item` (`opc_item_id`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `alarm_item_to_opc_item_read` FOREIGN KEY (`opc_item_id_read`) REFERENCES `nox`.`opc_item` (`opc_item_id`) ON UPDATE RESTRICT ON DELETE RESTRICT
) COLLATE='utf8_general_ci' ENGINE=InnoDB;
